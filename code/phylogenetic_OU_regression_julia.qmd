...
@model function vanilla_correlation(N)
    # Priors
    mu ~ MvNormal(zeros(2), 1)
    rho ~ Uniform(-1.0, 1.0)  # Correlation coefficient in (-0.95, 0.95)
...
nms = [nm for nm in sample_names(pt_vanilla_correlation) if !occursin("z", string(nm))]
...
posterior_chain = Turing.sample(vanilla_correlation(nrow(d)) | (x=d.x, y=d.y), NUTS(), length(chain_vanilla_correlation))

x_posterior = hcat(
    [rand(product_distribution(BernoulliLogit.(posterior_chain[:,"z[1, $i]", :]))) for i in 1:nrow(d)]...
)

y_posterior = hcat(
    [rand(product_distribution(BernoulliLogit.(posterior_chain[:,"z[2, $i]", :]))) for i in 1:nrow(d)]...
)
...
loglik_vanilla_correlation = hcat([logpdf.(BernoulliLogit.(posterior_chain[:,"z[1, $i]", :]), d.x[i]) .+ logpdf.(BernoulliLogit.(posterior_chain[:,"z[2, $i]", :]), d.y[i]) for i in 1:nrow(d)]...)
...
@model function brownian_correlation(scaled_cholesky_matrices, taxa)
    ...
    mu ~ MvNormal(zeros(2), 10)

    z_std ~ filldist(MvNormal(zeros(2), 1), length(taxa))
    z_cor = (Sigma_l * z_std) .* rates 
    z := z_cor * m .+ mu
...
chain_brownian_correlation = Chains(sample_array(pt_brownian_correlation), sample_names(pt_brownian_correlation))

nms = [nm for nm in sample_names(pt_brownian_correlation) if !occursin("z", string(nm))]

display(chain_brownian_correlation[:, nms, :])

plot(chain_brownian_correlation[:, nms, :])
...
# Prior predictive sampling 
prior_chain = Turing.sample(brownian_correlation(tree_cholesky, taxa), Prior(), length(chain_brownian_correlation))
...
chain_brownian_correlation = Turing.sample(
    brownian_correlation(tree_cholesky, taxa) | (x=d.x, y=d.y), 
    Gibbs(
        MH(:latent_index),
        NUTS(:mu, :rho, :rates, :z_std),
    ),
    MCMCThreads(),
    100,
    4,
)
chain_brownian_correlation[:, nms, :] |> display
