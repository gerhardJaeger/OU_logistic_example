...
@model function OU_correlation(scaled_phy_distances, taxa)
    n = length(scaled_phy_distances)
    rho_u ~ Normal()
    rho := 2 * cdf(Normal(), rho_u) - 1
    Sigma_l = [1 0.0; rho sqrt(1-rho^2)]
    tree_index_u ~ Normal()
    tree_index := Int(ceil(cdf(Normal(), tree_index_u) * n))
    m = scaled_phy_distances[tree_index]
    ...
    Sigma_OU_1 := sigma[1]^2/(2*lambda[1]) * exp.(-lambda[1] * m)
    Sigma_OU_2 := sigma[2]^2/(2*lambda[2]) * exp.(-lambda[2] * m)
    
    L_1 = cholesky(Sigma_OU_1).L
    L_2 = cholesky(Sigma_OU_2).L

    mu ~ MvNormal(zeros(2), 2)

    z_std ~ filldist(MvNormal(zeros(2), 1), length(taxa))
    z_cor = Sigma_l * z_std
    z_1 = L_1 * z_cor[1,:] .+ mu[1]
    z_2 = L_2 * z_cor[2,:] .+ mu[2]

    x ~ product_distribution(BernoulliLogit.(z_1))
    y ~ product_distribution(BernoulliLogit.(z_2))
end
...
chain_OU_correlation = Turing.sample(
    OU_correlation(scaled_cholesky_matrices, taxa) | (x=d.x, y=d.y),
    MH(),
    MCMCThreads(),
    num_warmup = 50_000,
    100_000,   # samples
