...
param_names_ci = [String(k) for k in keys(DynamicPPL.VarInfo(model_ctmc_indep))]
samples_ci = Array(chain_ctmc_indep[:, Symbol.(param_names_ci), :])
...
@model function ctmc_dependent(tip_states, ctmc_tree)
    ...
    # rates_to_Q fills upper triangle then lower triangle, row-major:
    # upper: r[1]=(0,0)→(0,1), r[2]=(0,0)→(1,0), r[3]=(0,1)→(1,1), r[4]=(1,0)→(1,1)
    # lower: r[5]=(0,1)→(0,0), r[6]=(1,0)→(0,0), r[7]=(1,1)→(0,1), r[8]=(1,1)→(1,0)
    Q    = rates_to_Q(rates, 4)
    stat = stationary_dist(Q)
...
param_names_cd = [String(k) for k in keys(DynamicPPL.VarInfo(model_ctmc_dep))]
samples_cd = Array(chain_ctmc_dep[:, Symbol.(param_names_cd), :])
