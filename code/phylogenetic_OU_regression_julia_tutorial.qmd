...
scaled_vcv = get_vcv(edge_tree) ./ 100  # divide by N (= 100 languages) so rate ~ LogNormal(0,1) is calibrated for logistic-scale outputs
...
### Helper functions: stationary distribution and Felsenstein pruning

The two shared helpers are (1) computing the stationary distribution of $Q$ via a linear solve, and (2) the Felsenstein pruning recursion. The $Q$ matrix is built directly inside each Turing model, where the independence constraints (or their absence) are most clearly expressed.
...
Exact conditional LOO log-likelihoods for a CTMC model. For each posterior
sample and each tip t, evaluates the Felsenstein likelihood with tip t
forced to each of its 4 possible states, then computes
log P(y_t | x_t, D_{-t}, params) as the log-ratio of joint likelihoods.
The p(D_{-t}) factor cancels exactly, so no approximation is involved.
Mirrors the `generated quantities` block in the Stan CTMC models.
...
```{julia}
plot(chain_ctmc_indep, size=(700, 500))
```
...
Under the dependent model, all 8 off-diagonal rates are free. We use 8 log-rates (Normal(0,1) prior) and the same pruning algorithm; the only difference from the independent model is the number of free parameters.
...
```{julia}
plot(chain_ctmc_dep, size=(700, 600))
```
...
The CTMC likelihood is defined over the joint tip configuration, not as a product of per-observation terms, so PSIS-LOO is not directly applicable. However, the exact conditional log-likelihood $\log p(y_t \mid x_t, D_{-t}, Q)$ can be computed for each tip without any approximation. The key identity is:

$$
\log p(y_t \mid x_t, D_{-t}, Q) = \log p(s_t^{\text{obs}}, D_{-t} \mid Q) - \log\!\left[p(s_t^{x,0}, D_{-t} \mid Q) + p(s_t^{x,1}, D_{-t} \mid Q)\right]
$$

where $s_t^{x,y}$ denotes the joint state $(x_t, y)$. Each of the three terms is a Felsenstein pruning likelihood with tip $t$ forced to a particular state; the $p(D_{-t} \mid Q)$ factor cancels in the ratio. This mirrors the `generated quantities` block in the Stan CTMC models. The resulting per-tip log-likelihoods are passed to `loo()`.

```{julia}
# Warning: ctmc_loo_loglik runs Felsenstein pruning 4× per tip per posterior sample.
# With 4000 samples × 100 tips × 4 pruning calls, this may take 15–30 minutes
# even with multithreading. Consider reducing the chain length during development.
...
Brownian motion is the $\lambda \to 0$ limit of the OU process. As a baseline, it captures phylogenetic covariance without mean-reversion — a comparison of OU vs. Brownian therefore directly tests whether stabilizing selection improves the fit, over and above phylogenetic control alone. The model is implemented via the precomputed Cholesky factor of the phylogenetic variance-covariance matrix (VCV):
...
The VCV Cholesky parameterization is convenient for Brownian motion because no per-edge decay factor is needed. The OU model below uses forward simulation along the tree instead — both are mathematically equivalent for Brownian motion, but forward simulation extends naturally to OU without requiring a matrix inverse.
...
We simulate the OU process node by node along the tree, starting from the root and propagating downward (forward simulation). This avoids inverting the covariance matrix, is more efficient, and extends naturally to the bivariate case.
...
println("Log BF (OU regression vs Brownian regression): ",
    logml_OU_regression - logml_brownian_regression)
```

The OU regression is strongly favored over both baselines. The Brownian comparison is the more informative one: since both models have identical phylogenetic structure, the Bayes factor directly quantifies the contribution of mean-reversion.
...
The Brownian motion correlation model provides the decisive comparison for the correlation models: if OU correlation outperforms Brownian correlation, the improvement cannot be attributed to phylogenetic structure per se — it specifically reflects the mean-reversion term. Brownian motion ($\lambda \to 0$) is the degenerate limit where both traits drift without bound; the Bayes factor between the two models directly quantifies how much stabilizing selection contributes beyond shared ancestry. We implement it via the VCV Cholesky, as in the Brownian regression above.
...
```{julia}
rhat(chain_brownian_correlation[:, nms_bc, :])
```

```{julia}
plot(chain_brownian_correlation[:, nms_bc, :], size=(700, 400))
```
...
Note that for the CTMC models, the per-tip conditional log-likelihoods $\log p(y_t \mid x_t, D_{-t}, Q)$ are computed exactly via the cancellation identity above (no importance sampling). For the continuous models, per-observation log-likelihoods are evaluated at the sampled latent states and then reweighted via PSIS. Despite the different computation strategies, `loo()` treats both identically for comparison.
