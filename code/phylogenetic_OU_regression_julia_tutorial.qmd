...
prior_cors_vc = filter(!isnan, [cor(
    Int.(prior_chain_vc[:, Symbol("x[$i]"), :]),
    Int.(prior_chain_vc[:, Symbol("y[$i]"), :])
) for i in 1:nrow(d)])

# Posterior predictive sampling
post_cors_vc = filter(!isnan, [cor(
    rand.(BernoulliLogit.(chain_vanilla_correlation[:, Symbol("z[1, $i]"), :])),
    rand.(BernoulliLogit.(chain_vanilla_correlation[:, Symbol("z[2, $i]"), :]))
) for i in 1:nrow(d)])
...
loglik_vanilla_correlation_y = hcat([
    logpdf.(BernoulliLogit.(chain_vanilla_correlation[:, Symbol("z[2, $i]"), :]), d.y[i])
    for i in 1:nrow(d)
]...)
...
loglik_brownian_regression = hcat([begin
    z_vals = Array(chain_brownian_regression[:, Symbol("z[$i]"), :])
    x_c = d.x .- mean(d.x)
    logpdf.(BernoulliLogit.(
...
x_prior_ouc = hcat([
    rand.(BernoulliLogit.(vec(Array(prior_chain_ouc[:, Symbol("x_logits[$k]"), :]))))
    for k in 1:nrow(d)
]...)
y_prior_ouc = hcat([
    rand.(BernoulliLogit.(vec(Array(prior_chain_ouc[:, Symbol("y_logits[$k]"), :]))))
    for k in 1:nrow(d)
]...)

x_post_ouc = hcat([
    rand.(BernoulliLogit.(vec(Array(chain_OU_correlation[:, Symbol("x_logits[$k]"), :]))))
    for k in 1:nrow(d)
]...)
y_post_ouc = hcat([
    rand.(BernoulliLogit.(vec(Array(chain_OU_correlation[:, Symbol("y_logits[$k]"), :]))))
    for k in 1:nrow(d)
]...)
...
loglik_OU_correlation_y = hcat([
    logpdf.(BernoulliLogit.(vec(Array(chain_OU_correlation[:, Symbol("y_logits[$i]"), :]))), d.y[i])
    for i in 1:nrow(d)
]...)
...
loglik_brownian_correlation_y = hcat([
    logpdf.(BernoulliLogit.(chain_brownian_correlation[:, Symbol("z[2, $i]"), :]), d.y[i])
    for i in 1:nrow(d)
]...)
